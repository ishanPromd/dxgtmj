  ...
  if (subjectsWithLessons.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-600 text-sm mb-4">You haven't acquired access to any premium lessons yet.</p>
      </div>
    );
  }
...
// Videos View Component with left/right gaps
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {lesson.videos.map((video, index) => (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: index * 0.1 }}
            className="group relative bg-white rounded-xl shadow-md border overflow-hidden hover:shadow-lg transition-all duration-300 cursor-pointer"
            onClick={(e) => {
              createRipple(e);
